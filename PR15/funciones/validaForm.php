...
function validaFormulario(){
    if(isset($_POST['Enviado'])){
        ...
        compruebaNombre('usuario');
        compruebaFecha('fecha');
        compruebaPass('pass1','pass2');
        compruebaEmail('email');

        if(compruebaNombre('usuario')&&compruebaPass('pass1','pass2')&&compruebaFecha('fecha')&&
            compruebaEmail('email')){
            return true;
        }else{
            return false;
...
function compruebaPass($pass1,$pass2){
    global $arrayError;
    if(!empty($_POST[$pass1]) && preg_match('/^\D{3,}$/',$_POST[$pass1])&& $_POST[$pass1]==$_POST[$pass2]){
        return true;
    }else{
